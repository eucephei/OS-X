...
// the number of iterations influeces fractal colorign and the time to compute the fractal
const int	 kMaximumIterations         =	2000;   // 300;
 // 2 is cited by as a reasonable limit for a good approximation of the limit for the mandlebrot equation
...
- (void) setRGBPixelData:(unsigned char*)pixelBuffer forX:(double)x_value Y:(double)y_value;
{
    // computing a fractal by interpreting x and y point as a complex number and testing for membership in the mandlebrot set
	// point is assumed to be in the set if its absolute value is greater than a specificed limit after a maximum number of iterations
	// this is an approximation of determining if recursive function goes to infinity for an infinite number of iterations of the loop
	// coloring comes from points not in the set and how many iterations were required for that point to pass the limit

    _Complex double c = x_value + (y_value * 1.0i);
    _Complex double z = 0;
 ...
 /** 
  This maps colors to a specific # of iterations of the loop before the result of the equation was deemed to go off to infinity 
  You can make adjustments here to change the colors displayed and adjust the resolution.
  **/
- (void) setPixelData:(unsigned char*)buffer forIterations:(int)iterations
{
	...
	if ( input <= (1.0f / 3.0f) )  
		...
		blue	= 1.0f;  // blue = 1.0f / green;	
	} 
    else {
		red		= powf((input - (2.0f / 3.0f) ) * kScaleFactor, 0.3333f);
		blue	= 1.0f;
		green	= 1.0f;  // green = 1.0f / red;
	}
